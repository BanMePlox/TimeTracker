...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Registrar nueva ausencia</h3>

        <form action="{{ route('admin.ausencias.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="3"
                          ...
                          placeholder="Observaciones o motivo...">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
